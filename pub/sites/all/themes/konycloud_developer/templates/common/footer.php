							...
							<ul>
								<?php foreach($item['#below'] as $subkey=>$subItem): 
									...
									if(substr($subItem['#href'],0,4) == 'node'){
										$subItemPath ='/'.drupal_lookup_path('alias',$subItem['#href']).trim($subItem['#localized_options']['attributes']['title']);
									}else{
										$subItemPath = $subItem['#href'].trim($subItem['#localized_options']['attributes']['title']);
									}
									?>
									<li>
										<a href="<?php echo $subItemPath == '<front>' ? '' : $subItemPath; ?>" title="<?php echo $subItemTitle; ?>">
